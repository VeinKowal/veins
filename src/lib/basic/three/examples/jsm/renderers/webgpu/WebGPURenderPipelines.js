...
class WebGPURenderPipelines {
  ...
  get(object) {
    ...
    if (pipeline === undefined) {
      ...
      let moduleVertex = this.shaderModules.vertex.get(
        nodeBuilder.vertexShader,
      );

      if (moduleVertex === undefined) {
        const byteCodeVertex = glslang.compileGLSL(
          nodeBuilder.vertexShader,
          'vertex',
        );
      ...
      let moduleFragment = this.shaderModules.fragment.get(
        nodeBuilder.fragmentShader,
      );

      if (moduleFragment === undefined) {
        const byteCodeFragment = glslang.compileGLSL(
          nodeBuilder.fragmentShader,
          'fragment',
        );
        ...
        this.shaderModules.fragment.set(
          nodeBuilder.fragmentShader,
          moduleFragment,
        );
      }
      ...
      const shaderAttributes = this._parseShaderAttributes(
        nodeBuilder.vertexShader,
      );
      ...
      pipeline = device.createRenderPipeline({
        ...
        fragment: Object.assign({}, moduleFragment, {
          targets: [
            {
              format: colorFormat,
              blend: {
  ...
  _parseShaderAttributes(shader) {
    ...
    const regex =
      /\s*layout\s*\(\s*location\s*=\s*(?<location>[0-9]+)\s*\)\s*in\s+(?<type>\w+)\s+(?<name>\w+)\s*;/gim;
