...
class USDZExporter {
  async parse(scene) {
    ...
    scene.traverse((object) => {
      if (object.isMesh) {
        ...
        if (material.normalMap !== null)
          textures[material.normalMap.uuid] = material.normalMap;
        if (material.aoMap !== null)
          textures[material.aoMap.uuid] = material.aoMap;
        if (material.roughnessMap !== null)
          textures[material.roughnessMap.uuid] = material.roughnessMap;
        if (material.metalnessMap !== null)
          textures[material.metalnessMap.uuid] = material.metalnessMap;
        if (material.emissiveMap !== null)
          textures[material.emissiveMap.uuid] = material.emissiveMap;
    ...
    for (const uuid in textures) {
      ...
      files['textures/Texture_' + texture.id + '.jpg'] = await imgToU8(
        texture.image,
      );
    }
...
function buildMaterial(material) {
  ...
  if (material.map !== null) {
    parameters.push(
      `${pad}color3f inputs:diffuseColor.connect = </Textures/Texture_${material.map.id}.outputs:rgb>`,
    );
  } else {
    parameters.push(
  ...
  if (material.emissiveMap !== null) {
    parameters.push(
      `${pad}color3f inputs:emissiveColor.connect = </Textures/Texture_${material.emissiveMap.id}.outputs:rgb>`,
    );
  ...
  if (material.normalMap !== null) {
    parameters.push(
      `${pad}normal3f inputs:normal.connect = </Textures/Texture_${material.normalMap.id}.outputs:rgb>`,
    );
  }

  if (material.aoMap !== null) {
    parameters.push(
      `${pad}float inputs:occlusion.connect = </Textures/Texture_${material.aoMap.id}.outputs:r>`,
    );
  }

  if (material.roughnessMap !== null) {
    parameters.push(
      `${pad}float inputs:roughness.connect = </Textures/Texture_${material.roughnessMap.id}.outputs:g>`,
    );
  } else {
  ...
  if (material.metalnessMap !== null) {
    parameters.push(
      `${pad}float inputs:metallic.connect = </Textures/Texture_${material.metalnessMap.id}.outputs:b>`,
    );
  } else {
