...
THREE.BasisTextureLoader.prototype = Object.assign(
  Object.create(THREE.Loader.prototype),
  {
    ...
    _createTexture: function (buffers, config) {
      ...
      THREE.BasisTextureLoader.taskCache.set(buffers[0], {
        ...
        this.transcoderPending = Promise.all([jsContent, binaryContent]).then(
          ([jsContent, binaryContent]) => {
            ...
            var body = [
              '/* constants */',
              'var _EngineFormat = ' +
                JSON.stringify(THREE.BasisTextureLoader.EngineFormat),
              'var _TranscoderFormat = ' +
                JSON.stringify(THREE.BasisTextureLoader.TranscoderFormat),
              'var _BasisFormat = ' +
                JSON.stringify(THREE.BasisTextureLoader.BasisFormat),
              '/* basis_transcoder.js */',
              jsContent,
              '/* worker */',
