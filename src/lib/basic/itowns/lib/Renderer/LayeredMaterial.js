...
exports['default'] =
  exports.ELEVATION_MODES =
  exports.colorLayerEffects =
    void 0;
  ...
  if (count > max) {
    console.warn(
      'LayeredMaterial: Not enough texture units ('
        .concat(max, ' < ')
        .concat(count, '), excess textures have been discarded.'),
    );
  }
...
var LayeredMaterial = /*#__PURE__*/ (function (_THREE$RawShaderMater) {
  ...
  function LayeredMaterial() {
    ...
    nbSamplers = nbSamplers || [
      samplersElevationCount,
    ...
    _CommonMaterial['default'].setDefineMapping(
    ...
    _CommonMaterial['default'].setDefineMapping(
      (0, _assertThisInitialized2['default'])(_this),
      'MODE',
      _RenderMode['default'].MODES,
    );

    _CommonMaterial['default'].setDefineProperty(
      (0, _assertThisInitialized2['default'])(_this),
      'mode',
      'MODE',
      _RenderMode['default'].MODES.FINAL,
    );
    ...
    fragmentShader[crsCount] =
      fragmentShader[crsCount] ||
      _ShaderUtils['default'].unrollLoops(TileFS, _this.defines);
    _this.fragmentShader = fragmentShader[crsCount]; // Color uniforms

    _CommonMaterial['default'].setUniformProperty(
      (0, _assertThisInitialized2['default'])(_this),
      'diffuse',
      new THREE.Color(0.04, 0.23, 0.35),
    );
    ...
    _this.uniforms.elevationLayers = new THREE.Uniform(
      new Array(nbSamplers[0]).fill(defaultStructLayer),
    );
    _this.uniforms.elevationTextures = new THREE.Uniform(
      new Array(nbSamplers[0]).fill(defaultTex),
    );
    _this.uniforms.elevationOffsetScales = new THREE.Uniform(
      new Array(nbSamplers[0]).fill(identityOffsetScale),
    );
    _this.uniforms.elevationTextureCount = new THREE.Uniform(0); // color layer uniforms, to be updated using updateUniforms()

    _this.uniforms.colorLayers = new THREE.Uniform(
      new Array(nbSamplers[1]).fill(defaultStructLayer),
    );
    _this.uniforms.colorTextures = new THREE.Uniform(
      new Array(nbSamplers[1]).fill(defaultTex),
    );
    _this.uniforms.colorOffsetScales = new THREE.Uniform(
      new Array(nbSamplers[1]).fill(identityOffsetScale),
    );
    ...
    },
    {
      ...
      value: function updateLayersUniforms() {
        ...
        var colorlayers = this.layers.filter(function (l) {
          return (
            _this2.colorLayerIds.includes(l.id) && l.visible && l.opacity > 0
          );
        });
        colorlayers.sort(function (a, b) {
          return (
            _this2.colorLayerIds.indexOf(a.id) -
            _this2.colorLayerIds.indexOf(b.id)
          );
        });

        _updateLayersUniforms(
