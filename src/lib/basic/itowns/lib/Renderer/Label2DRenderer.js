...
function hasLabelChildren(object) {
  ...
  return (
    ...
    parent.children.find(function (c) {
      return (
        c.isTileMesh &&
        c.children.find(function (cc) {
          return cc.isLabel;
        })
      );
    })
  ...
  (0, _createClass2['default'])(ScreenGrid, [
    ...
    },
    {
      ...
      value: function insert(obj) {
        var minx = Math.max(
          0,
          Math.floor((obj.boundaries.left / this.width) * this.x),
        );
        var maxx = Math.min(
          this.x - 1,
          Math.floor((obj.boundaries.right / this.width) * this.x),
        );
        var miny = Math.max(
          0,
          Math.floor((obj.boundaries.top / this.height) * this.y),
        );
        var maxy = Math.min(
          this.y - 1,
          Math.floor((obj.boundaries.bottom / this.height) * this.y),
        );
  ...
  (0, _createClass2['default'])(Label2DRenderer, [
    ...
    },
    {
      ...
      value: function render(scene, camera) {
        ...
        if (
          !this.infoTileLayer ||
          !this.infoTileLayer.layer.attachedLayers.find(function (l) {
    ...
    },
    {
      ...
      value: function culling(object, extent) {
        ...
        if (!object.isLabel) {
          ...
          !extent.isPointInside(object.coordinates) ||
          ...
          object.parent.level < 4 &&
          object.parent.layer.horizonCulling &&
          object.parent.layer.horizonCulling(object.horizonCullingPoint)
        ) {
          ...
          object.updateProjectedPosition(
            vector.x * this.halfWidth + this.halfWidth,
            -vector.y * this.halfHeight + this.halfHeight,
          ); // Are considered duplicates, labels that have the same screen
          // coordinates and the same base content.

          if (
            this.grid.visible.some(function (l) {
              return (
                l.projectedPosition.x == object.projectedPosition.x &&
