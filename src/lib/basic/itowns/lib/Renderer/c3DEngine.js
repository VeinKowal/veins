...
var c3DEngine = /*#__PURE__*/function () {
  function c3DEngine(rendererOrDiv) {
    ...
    try {
      ...
      this.label2dRenderer.setSize(this.width, this.height);
      for (var i = 0; i < viewerDiv.childNodes.length; i += 1) {
        var childEl = viewerDiv.childNodes[i];
        if (childEl instanceof HTMLDivElement) {
          this.label2dRenderer.domElement.append(viewerDiv.childNodes[i]);
        }
      }
