...
var c3DEngine = /*#__PURE__*/ (function () {
  function c3DEngine(rendererOrDiv) {
    ...
    this.fullSizeRenderTarget = new THREE.WebGLRenderTarget(
      this.width,
      this.height,
    );
    ...
    try {
      ...
      this.renderer =
        renderer ||
        new (options.isWebGL2 ? THREE.WebGLRenderer : THREE.WebGL1Renderer)({
      ...
      console.error(
        'Failed to create WebGLRenderer webGL '.concat(versionWebGL, '.'),
      );
      this.renderer = null;
    }
    ...
    if (!renderer && options.logarithmicDepthBuffer) {
      ...
      if (
        ...
        this.renderer = new (
          options.isWebGL2 ? THREE.WebGLRenderer : THREE.WebGL1Renderer
        )({
          canvas: document.createElement('canvas'),
          antialias: options.antialias,
          alpha: options.alpha,
  ...
  (0, _createClass2['default'])(c3DEngine, [
    ...
    },
    {
      ...
      value: function renderViewToBuffer(view, zone) {
        ...
        zone.buffer =
          zone.buffer || new Uint8Array(4 * zone.width * zone.height);
