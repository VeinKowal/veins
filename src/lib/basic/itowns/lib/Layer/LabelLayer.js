      ...
      value: function convert(data, extent) {
        ...
        data.features.forEach(function (f) {
          ...
          f.geometries.forEach(function (g) {
            ...
            var geometryField =
              g.properties.style && g.properties.style.text.field;
            var content;
            var context = {
              globals: globals,
            ...
            if (!geometryField && !featureField && !layerField) {
              ...
              if (
                ...
                !(_this2.style && _this2.style.icon)
              ) {
                return;
              }
    ...
    },
    {
      ...
      value: function update(context, layer, node, parent) {
        ...
        if (!parent && node.children.length) {
          ...
          _ObjectRemovalHelper['default'].removeChildrenAndCleanupRecursively(
            this,
            node,
          );
        ...
        var extentsDestination = node.getExtentsByProjection(
          this.source.crs,
        ) || [node.extent];
        ...
        if (
          elevationLayer &&
          node.layerUpdateState[elevationLayer.id].canTryUpdate()
        ) {
          node.children.forEach(function (c) {
            if (
              c.isLabel &&
              c.needsAltitude &&
              c.updateElevationFromLayer(_this3.parent)
            ) {
        ...
        return context.scheduler.execute(command).then(function (result) {
          ...
          result.forEach(function (labels) {
            if (!node.parent) {
              labels.forEach(function (l) {
                _ObjectRemovalHelper[
                  'default'
                ].removeChildrenAndCleanupRecursively(_this3, l);
          ...
          if (labelsDiv.length > 0) {
            ...
            (_nodeDomElement$dom = nodeDomElement.dom).append.apply(
              _nodeDomElement$dom,
              labelsDiv,
            );
            ...
            (
              (closestDomElement && closestDomElement.dom) ||
              _this3.domElement
            ).appendChild(nodeDomElement.dom);
