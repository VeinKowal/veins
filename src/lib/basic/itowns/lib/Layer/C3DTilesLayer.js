...
var C3DTilesLayer = /*#__PURE__*/ (function (_GeometryLayer) {
  ...
  function C3DTilesLayer(id, config, view) {
    ...
    _this.overrideMaterials =
      config.overrideMaterials !== undefined ? config.overrideMaterials : true;
    _this.name = config.name;
    _this.registeredExtensions =
      config.registeredExtensions || new _C3DTExtensions['default']();
    _this._cleanableTiles = [];

    var resolve = _this.addInitializationStep();

    _this.source.whenReady.then(function (tileset) {
      _this.tileset = new _C3DTileset['default'](
        tileset,
        _this.source.baseUrl,
        _this.registeredExtensions,
      ); // Verify that extensions of the tileset have been registered in the layer

      if (_this.tileset.extensionsUsed) {
        var _iterator = _createForOfIteratorHelper(
            _this.tileset.extensionsUsed,
          ),
          _step;
        ...
        try {
          for (_iterator.s(); !(_step = _iterator.n()).done; ) {
            ...
            if (
              !_this.registeredExtensions.isExtensionRegistered(extensionUsed)
            ) {
              // if it is required to load the tileset
              if (
                _this.tileset.extensionsRequired &&
                _this.tileset.extensionsRequired.includes(extensionUsed)
              ) {
                console.error(
                  '3D Tiles tileset required extension "'.concat(
                    extensionUsed,
                    '" must be registered to the 3D Tiles layer of iTowns to be parsed and used.',
                  ),
                );
              } else {
                console.warn(
                  '3D Tiles tileset used extension "'.concat(
                    extensionUsed,
                    '" must be registered to the 3D Tiles layer of iTowns to be parsed and used.',
                  ),
                );
      ...
      (0, _dTilesProcessing.init3dTilesLayer)(
        view,
