...
var _OrientedImageMaterial = _interopRequireDefault(
  require('../Renderer/OrientedImageMaterial'),
);
...
function updatePano(context, camera, layer) {
  ...
  if (newPano && currentId != newPano.id) {
    ...
    layer.onPanoChanged({
      previousPanoPosition: layer.getPreviousPano()
        ? layer.getPreviousPano().position
        : undefined,
    ...
    context.scheduler.execute(command).then(
      function (textures) {
        if (newPano.id === layer.currentPano.id) {
          layer.material.setTextures(
            textures,
            newPano,
            layer.getCamerasNameFromFeature(newPano),
          );
...
var OrientedImageLayer = /*#__PURE__*/ (function (_GeometryLayer) {
  ...
  function OrientedImageLayer(id) {
    ...
    if (config.projection) {
      console.warn(
        'OrientedImageLayer projection parameter is deprecated, use crs instead.',
      );
    ...
    _this.background =
      config.background || createBackground(config.backgroundDistance);
    ...
    _this.getCamerasNameFromFeature =
      config.getCamerasNameFromFeature || function () {};
    ...
    _this.source.whenReady.then(function (metadata) {
      return _GeoJsonParser['default']
        .parse(config.orientation || metadata.orientation, options)
        .then(function (orientation) {
          ...
          var crs2crs = _OrientationUtils['default'].quaternionFromCRSToCRS(
            crsIn,
            crsOut,
          );
          ...
          try {
            for (_iterator.s(); !(_step = _iterator.n()).done; ) {
              ...
              coord.crs = pano.crs;
              coord
                .setFromArray(pano.vertices)
                .applyMatrix4(orientation.matrix);
              pano.position = coord.toVector3(); // set quaternion

              crs2crs(coord, quat);
              pano.quaternion = _OrientationUtils['default']
                .quaternionFromAttitude(pano.geometries[0].properties)
                .premultiply(quat);
        ...
        .then(function () {
          ...
          _CameraCalibrationParser['default']
            .parse(config.calibration || metadata.calibration, config)
            .then(function (cameras) {
              _this.cameras = cameras; // create the material

              _this.material = new _OrientedImageMaterial['default'](
                _this.cameras,
                config,
              );
              resolve();
  ...
  (0, _createClass2['default'])(OrientedImageLayer, [
    ...
    },
    {
      ...
      value: function _delete() {
        (0, _get2['default'])(
          (0, _getPrototypeOf2['default'])(OrientedImageLayer.prototype),
          'delete',
          this,
        ).call(this);
        this.material.visible = false;
        console.warn(
          'You need to replace OrientedImageLayer.material applied on each object. This issue will be fixed when OrientedImageLayer will be a ColorLayer. the material visibility is set to false. To follow issue see https://github.com/iTowns/itowns/issues/1018',
        );
      },
    },
    {
