...
var _convertToTile = _interopRequireDefault(
  require('../Converter/convertToTile'),
);

var _ObjectRemovalHelper = _interopRequireDefault(
  require('../Process/ObjectRemovalHelper'),
);
...
var TiledGeometryLayer = /*#__PURE__*/ (function (_GeometryLayer) {
  ...
  function TiledGeometryLayer(id, object3d, schemeTile, builder, config) {
    ...
    _this.info = new _InfoLayer.InfoTiledGeometryLayer(
      (0, _assertThisInitialized2['default'])(_this),
    );

    if (!_this.schemeTile) {
      throw new Error(
        'Cannot init tiled layer without schemeTile for layer '.concat(
          _this.id,
        ),
      );
    }
    ...
    if (!_this.builder) {
      throw new Error(
        'Cannot init tiled layer without builder for layer '.concat(_this.id),
      );
    }
    ...
    _this._promises.push(
      Promise.all(promises).then(function (level0s) {
        var _this$object3d;

        _this.level0Nodes = level0s;

        (_this$object3d = _this.object3d).add.apply(
          _this$object3d,
          (0, _toConsumableArray2['default'])(level0s),
        );
    ...
    [
        ...
        value: function pickObjectsAt(view, coordinates) {
          ...
          var target =
            arguments.length > 3 && arguments[3] !== undefined
              ? arguments[3]
              : [];
          return _Picking['default'].pickTilesAt(
            view,
            coordinates,
            radius,
            this,
        ...
        value: function preUpdate(context, sources) {
          ...
          try {
            for (_iterator2.s(); !(_step2 = _iterator2.n()).done; ) {
              var e = _step2.value;
              context.maxElevationLevel = Math.max(
                e.source.zoom.max,
                context.maxElevationLevel,
              );
            }
          } catch (err) {
            _iterator2.e(err);
          ...
          this.colorLayersOrder =
            _Layer.ImageryLayers.getColorLayersIdOrderedBySequence(
              context.colorLayers,
            );
          var commonAncestor;
      ...
      },
      {
        ...
        value: function update(context, layer, node) {
          ...
          if (!node.parent) {
            return _ObjectRemovalHelper['default'].removeChildrenAndCleanup(
              this,
              node,
            );
          ...
          if (node.visible) {
            ...
            if (
              ...
              (TiledGeometryLayer.hasEnoughTexturesToSubdivide(context, node) &&
                this.subdivision(context, this, node))
            ) {
            ...
            if (node.material.visible) {
              if (!requestChildrenUpdate) {
                return _ObjectRemovalHelper['default'].removeChildren(
                  this,
                  node,
                );
              }
      ...
      },
      {
        ...
        value: function subdivision(context, layer, node) {
          ...
          boundingSphereCenter
            .copy(node.boundingSphere.center)
            .applyMatrix4(node.matrixWorld);
          var distance = Math.max(
            0.0,
            context.camera.camera3D.position.distanceTo(boundingSphereCenter) -
              node.boundingSphere.radius * subdivisionVector.x,
          ); // Size projection on pixel of bounding

          if (context.camera.camera3D.isOrthographicCamera) {
            var camera3D = context.camera.camera3D;
            var preSSE =
              (context.camera._preSSE * 2 * camera3D.zoom) /
              (camera3D.top - camera3D.bottom);
            node.screenSize =
              preSSE * node.boundingSphere.radius * subdivisionVector.x;
          } else {
            node.screenSize =
              (context.camera._preSSE *
                (2 * node.boundingSphere.radius * subdivisionVector.x)) /
              distance;
          ...
          var sse =
            node.screenSize /
            (_LayeredMaterialNodeProcessing.SIZE_DIAGONAL_TEXTURE * 2);
        ...
        value: function hasEnoughTexturesToSubdivide(context, node) {
          ...
          try {
            for (_iterator5.s(); !(_step5 = _iterator5.n()).done; ) {
              ...
              if (
                !e.frozen &&
                e.ready &&
                e.source.extentInsideLimit(node.extent, zoom) &&
                (!nodeLayer || nodeLayer.level < 0)
              ) {
                // no stop subdivision in the case of a loading error
                if (
                  layerUpdateState[e.id] &&
                  layerUpdateState[e.id].inError()
                ) {
                  continue;
                }
          ...
          try {
            for (_iterator6.s(); !(_step6 = _iterator6.n()).done; ) {
              ...
              if (
                c.source.extentInsideLimit(node.extent, _zoom) &&
                (!nodeLayer || nodeLayer.level < 0)
              ) {
                return false;
              }
