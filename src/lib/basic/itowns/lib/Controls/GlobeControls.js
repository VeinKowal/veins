...
var _AnimationPlayer = _interopRequireDefault(
  require('../Core/AnimationPlayer'),
);
...
var GlobeControls = /*#__PURE__*/ (function (_THREE$EventDispatche) {
  ...
  function GlobeControls(view, placement) {
    ...
    if (options.zoomSpeed) {
      console.warn(
        'Controls zoomSpeed parameter is deprecated. Use zoomInFactor and zoomOutFactor instead.',
      );
    ...
    _this.maxDistance =
      options.maxDistance || _Ellipsoid.ellipsoidSizes.x * 8.0; // Limits to how far you can zoom in and out ( OrthographicCamera only )
    ...
    _this.dampingMoveFactor =
      options.dampingMoveFactor != undefined
        ? options.dampingMoveFactor
    ...
    _this.updateHelper = enableTargetHelper
      ? function (position, helper) {
    ...
    _this._onContextMenuListener = _this.onContextMenuListener.bind(
      (0, _assertThisInitialized2['default'])(_this),
    );
    _this._ondblclick = _this.ondblclick.bind(
      (0, _assertThisInitialized2['default'])(_this),
    );
    _this._onTouchStart = _this.onTouchStart.bind(
      (0, _assertThisInitialized2['default'])(_this),
    );
    _this._update = _this.update.bind(
      (0, _assertThisInitialized2['default'])(_this),
    );
    _this._onTouchMove = _this.onTouchMove.bind(
      (0, _assertThisInitialized2['default'])(_this),
    );
    ...
    _this._onBlurListener = _this.onBlurListener.bind(
      (0, _assertThisInitialized2['default'])(_this),
    );

    _this.view.domElement.addEventListener(
      'contextmenu',
      _this._onContextMenuListener,
      false,
    );
  ...
  (0, _createClass2['default'])(GlobeControls, [
    ...
    },
    {
      ...
      value: function mouseToPan(deltaX, deltaY) {
        ...
        if (this.camera.isPerspectiveCamera) {
          var targetDistance = this.camera.position.distanceTo(
            this.getCameraTargetPosition(),
          ); // half of the fov is center to top of screen

          targetDistance *=
            2 * Math.tan(THREE.MathUtils.degToRad(this.camera.fov * 0.5)); // we actually don't use screenWidth, since perspective camera is fixed to screen height

          this.panLeft(
            ((deltaX * targetDistance) / gfx.width) * this.camera.aspect,
          );
          this.panUp((deltaY * targetDistance) / gfx.height);
        } else if (this.camera.isOrthographicCamera) {
          // orthographic
          this.panLeft(
            (deltaX * (this.camera.right - this.camera.left)) / gfx.width,
          );
          this.panUp(
            (deltaY * (this.camera.top - this.camera.bottom)) / gfx.height,
          );
        }
      },
    },
      ...
      value: function getMinDistanceCameraBoundingSphereObbsUp(tile) {
        if (tile.level > 10 && tile.children.length == 1 && tile.geometry) {
          ...
          if (obb.isSphereAboveXYBox(sphereCamera)) {
            minDistanceZ = Math.min(
              sphereCamera.center.z - obb.box3D.max.z,
              minDistanceZ,
            );
          }
        }
      ...
      value: function update() {
        ...
        if (this.handleCollision) {
          ...
          if (this.view.tileLayer) {
            var _iterator = _createForOfIteratorHelper(
                this.view.tileLayer.level0Nodes,
              ),
              _step;
            ...
            try {
              for (_iterator.s(); !(_step = _iterator.n()).done; ) {
                ...
                tile.traverse(
                  this.getMinDistanceCameraBoundingSphereObbsUp.bind(this),
                );
              }
            } catch (err) {
              _iterator.e(err);
        ...
        switch (this.state) {
          ...
          case this.states.MOVE_GLOBE:
            if (minDistanceZ < 0) {
              ...
              this.camera.position.setLength(
                this.camera.position.length() - minDistanceZ,
              );
            } else if (minDistanceZ < this.minDistanceCollision) {
              var translate =
                this.minDistanceCollision *
                (1.0 - minDistanceZ / this.minDistanceCollision);
              cameraTarget.translateY(translate);
              this.camera.position.setLength(
                this.camera.position.length() + translate,
              );
            }

            lastNormalizedIntersection
              .copy(normalizedIntersection)
              .applyQuaternion(moveAroundGlobe);
          ...
          case this.states.PANORAMIC: {
            ...
            var normal = this.camera.position
              .clone()
              .normalize()
              .applyQuaternion(this.camera.quaternion.clone().invert());
            quaterPano
              .setFromAxisAngle(normal, sphericalDelta.theta)
              .multiply(quaterAxis.setFromAxisAngle(axisX, sphericalDelta.phi));
          ...
          default: {
            ...
            if (this.handleCollision) {
              if (
                minDistanceZ < slowdownLimit &&
                minDistanceZ > contraryLimit &&
                ...
                var slowdownFactor =
                  1 -
                  (slowdownZone - (minDistanceZ - contraryLimit)) /
                    slowdownZone; // apply slowdown factor on tilt mouvement

                sphericalDelta.phi *= slowdownFactor * slowdownFactor;
              } else if (
                minDistanceZ < contraryLimit &&
                minDistanceZ > -contraryLimit &&
                sphericalDelta.phi > minContraintPhi
              ) {
                // contraint zone : contraint sphericalDelta.phi
                // calculation of the angle of rotation which allows to leave this zone
                var contraryPhi = -Math.asin(
                  ((contraryLimit - minDistanceZ) * 0.25) / spherical.radius,
                ); // clamp contraryPhi to make a less brutal exit

                contraryPhi = THREE.MathUtils.clamp(
                  contraryPhi,
                  minContraintPhi,
                  0,
                ...
                var contraryFactor =
                  1 - (contraryLimit - minDistanceZ) / (2 * contraryLimit);
                sphericalDelta.phi = THREE.MathUtils.lerp(
                  sphericalDelta.phi,
                  contraryPhi,
                  contraryFactor,
                );
        ...
        if (
          ...
          (sphericalDelta.theta > EPS || sphericalDelta.phi > EPS)
        ) {
      ...
      value: function onMouseMove(event) {
        ...
        switch (this.state) {
          ...
          case this.states.PANORAMIC: {
            ...
            var gfx = this.view.mainLoop.gfxEngine;
            this.rotateLeft(
              ((2 * Math.PI * rotateDelta.x) / gfx.width) * this.rotateSpeed,
            ); // rotating up and down along whole screen attempts to go 360, but limited to 180

            this.rotateUp(
              ((2 * Math.PI * rotateDelta.y) / gfx.height) * this.rotateSpeed,
            );
          ...
          case this.states.MOVE_GLOBE: {
            ...
            if (raycaster.ray.intersectSphere(pickSphere, intersection)) {
              ...
              moveAroundGlobe.setFromUnitVectors(
    ...
    },
    {
      ...
      value: function updateTarget() {
        ...
        var distance = !isNaN(pickedPosition.x)
          ? this.camera.position.distanceTo(pickedPosition)
          : 100;
    ...
    },
    {
      ...
      value: function handlingEvent(current) {
        current =
          current ||
          _CameraUtils['default'].getTransformCameraLookingAtTarget(
            this.view,
            this.camera,
          );
    ...
    },
    {
      ...
      value: function ondblclick(event) {
        ...
        this.player.stop();
        var point = this.view.getPickingPositionFromDepth(
      ...
      value: function onMouseUp() {
        ...
        if (this.enableDamping) {
          if (
            ...
            this.state === this.states.MOVE_GLOBE &&
            Date.now() - lastTimeMouseMove < 50
          ) {
    ...
    },
    {
      ...
      value: function getRange(position) {
        return _CameraUtils['default'].getTransformCameraLookingAtTarget(
          this.view,
          this.camera,
          position,
        ).range;
    ...
    },
    {
      ...
      value: function getCameraCoordinate() {
        return new _Coordinates['default'](
          'EPSG:4978',
    ...
    },
    {
      ...
      value: function getZoom() {
        return this.view.tileLayer.computeTileZoomFromDistanceCamera(
          this.getRange(),
          this.view.camera,
        );
      },
    ...
    {
      key: 'getScale',
      value: function getScale(pitch) /* istanbul ignore next */
      {
    ...
    {
      key: 'pixelsToMeters',
      value: function pixelsToMeters(pixels) /* istanbul ignore next */
      {
        var pixelPitch =
    ...
    {
      key: 'pixelsToDegrees',
      value: function pixelsToDegrees(pixels) /* istanbul ignore next */
      {
        var pixelPitch =
          arguments.length > 1 && arguments[1] !== undefined
            ? arguments[1]
            : 0.28;
        console.warn(
          'Deprecated, use View#getPixelsToMeters and GlobeControls#getMetersToDegrees instead.',
        );
        var chord = this.pixelsToMeters(pixels, pixelPitch);
        return THREE.MathUtils.radToDeg(
          2 * Math.asin(chord / (2 * _Ellipsoid.ellipsoidSizes.x)),
        );
      },
      /**
    ...
    {
      key: 'metersToPixels',
      value: function metersToPixels(value) /* istanbul ignore next */
      {
        var pixelPitch =
          arguments.length > 1 && arguments[1] !== undefined
            ? arguments[1]
    ...
    },
    {
      ...
      value: function lookAtCoordinate() {
        ...
        if (!params.isExtent) {
          if (params.zoom) {
            params.range =
              this.view.tileLayer.computeDistanceCameraFromTileZoom(
                params.zoom,
                this.view.camera,
              );
          } else if (params.scale) {
            params.range = this.view.getScaleFromDistance(
              params.pitch,
              params.scale,
            );

            if (
              params.range < this.minDistance ||
              params.range > this.maxDistance
            ) {
              // eslint-disable-next-line no-console
              console.warn(
                'This scale '.concat(params.scale, ' can not be reached'),
              );
              params.range = THREE.MathUtils.clamp(
                params.range,
                this.minDistance,
                this.maxDistance,
              );
          ...
          if (params.tilt !== undefined) {
            ...
            if (params.tilt < minTilt || params.tilt > maxTilt) {
              params.tilt = THREE.MathUtils.clamp(
                params.tilt,
                minTilt,
                maxTilt,
              ); // eslint-disable-next-line no-console
        ...
        if (isAnimated) {
          ...
          return _CameraUtils['default']
            .animateCameraToLookAtTarget(this.view, this.camera, params)
          ...
          return _CameraUtils['default']
            .transformCameraToLookAtTarget(this.view, this.camera, params)
            .then(function (result) {
    ...
    },
    {
      ...
      value: function pickGeoPosition(windowCoords) {
        var pickedPosition =
          this.view.getPickingPositionFromDepth(windowCoords);

        if (!pickedPosition) {
          return;
        }

        return new _Coordinates['default']('EPSG:4978', pickedPosition).as(
          'EPSG:4326',
        );
      },
    },
