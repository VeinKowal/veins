...
var toFeature = {
  populateGeometry: function populateGeometry(
    crsIn,
    coordinates,
    geometry,
    feature,
  ...
  default: function _default(feature, crsIn, coordsIn, collection, properties) {
    if (
      collection.filterExtent &&
      firstPtIsOut(collection.filterExtent, coordsIn, crsIn)
    ) {
      return;
    }
    ...
    geometry.properties.style = new _Style['default'](
      {},
      feature.style,
    ).setFromGeojsonProperties(properties, feature.type);
  ...
  polygon: function polygon(feature, crsIn, coordsIn, collection, properties) {
    ...
    if (
      collection.filterExtent &&
      firstPtIsOut(collection.filterExtent, coordsIn[0], crsIn)
    ) {
      return;
    }
    ...
    geometry.properties.style = new _Style['default'](
      {},
      feature.style,
    ).setFromGeojsonProperties(properties, feature.type); // Then read contour and holes
...
function jsonFeatureToFeature(crsIn, json, collection) {
  ...
  for (
    var _i = 0, _Object$keys = Object.keys(json);
    ...
    switch (json.type.toLowerCase()) {
      case 'featurecollection':
        return Promise.resolve(
          jsonFeaturesToFeatures(_in.crs, json.features, out),
        );
