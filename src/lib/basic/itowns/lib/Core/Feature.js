...
exports.FeatureCollection =
  exports['default'] =
  exports.FeatureGeometry =
  exports.FeatureBuildingOptions =
    ...
    },
    {
      ...
      value: function pushCoordinates(coordIn, feature) {
        if (this.size == 3) {
          ...
          var base_altitude =
            feature.style[typeToStyleProperty[feature.type]].base_altitude;
          coordIn.z = isNaN(base_altitude)
            ? base_altitude(this.properties, coordIn)
            : base_altitude;
        }
        ...
        if (this._currentExtent) {
          this._currentExtent.expandByCoordinates(
            feature.useCrsOut ? coordOut : coordIn,
          );
        }
...
var Feature = /*#__PURE__*/ (function () {
  ...
  function Feature(type, collection) {
    ...
    if (
      Object.keys(FEATURE_TYPES).find(function (t) {
    ...
    this.transformToLocalSystem =
      collection.transformToLocalSystem.bind(collection);
    ...
    this._pos = 0;
    this._pushValues = (this.size === 3 ? push3DValues : push2DValues).bind(
      this,
    );
      ...
      value: function updateExtent(geometry) {
        ...
        if (this.size == 3) {
          this.altitude.min = Math.min(
            this.altitude.min,
            geometry.altitude.min,
          );
          this.altitude.max = Math.max(
            this.altitude.max,
            geometry.altitude.max,
          );
        }
      },
      /**
...
var FeatureCollection = /*#__PURE__*/ (function (_THREE$Object3D) {
  ...
  function FeatureCollection(options) {
    ...
    _this.features = [];
    _this.mergeFeatures =
      options.mergeFeatures === undefined ? true : options.mergeFeatures;
    _this.extent = options.buildExtent
      ? defaultExtent(options.forcedExtentCrs || _this.crs)
      : undefined;
    ...
    if (_this.size == 2) {
      ...
      _this._setLocalSystem = function (center) {
        ...
        if (_this.crs == 'EPSG:4978') {
          ...
          alignYtoEast.setFromAxisAngle(
            axisZ,
            THREE.MathUtils.degToRad(90 + _this.center.longitude),
          );
        ...
        _this.normalMatrixInverse = new THREE.Matrix3()
          .copy(_this.normalMatrix)
          .invert();
        _this._setLocalSystem = doNothing;
      };
  ...
  (0, _createClass2['default'])(FeatureCollection, [
    ...
    },
    {
      ...
      value: function updateExtent(extent) {
        ...
        if (this.size == 3) {
          ...
          try {
            for (_iterator2.s(); !(_step2 = _iterator2.n()).done; ) {
              ...
              this.altitude.min = Math.min(
                this.altitude.min,
                feature.altitude.min,
              );
              this.altitude.max = Math.max(
                this.altitude.max,
                feature.altitude.max,
              );
            }
          } catch (err) {
            _iterator2.e(err);
        ...
        this.altitude.min =
          this.altitude.min == Infinity ? 0 : this.altitude.min;
        this.altitude.max =
          this.altitude.max == -Infinity ? 0 : this.altitude.max;
      },
      /**
    ...
    },
    {
      ...
      value: function updateMatrixWorld(force) {
        (0, _get2['default'])(
          (0, _getPrototypeOf2['default'])(FeatureCollection.prototype),
          'updateMatrixWorld',
          this,
        ).call(this, force);
