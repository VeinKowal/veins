  ...
  (0, _createClass2['default'])(PotreeNode, [
    {
      ...
      value: function add(node, indexChild, root) {
        (0, _get2['default'])(
          (0, _getPrototypeOf2['default'])(PotreeNode.prototype),
          'add',
          this,
        ).call(this, node, indexChild);
        ...
        if (node.id.length % this.layer.hierarchyStepSize == 0) {
          node.baseurl = ''
            .concat(root.baseurl, '/')
            .concat(node.id.substr(root.id.length));
        } else {
    ...
    },
    {
      ...
      get: function get() {
        return ''
          .concat(this.baseurl, '/r')
          .concat(this.id, '.')
          .concat(this.layer.source.extension);
      },
    },
    {
      key: 'loadOctree',
      value: function loadOctree() {
        ...
        var octreeUrl = ''
          .concat(this.baseurl, '/r')
          .concat(this.id, '.')
          .concat(this.layer.source.extensionOctree);
        return this.layer.source
          .fetcher(octreeUrl, this.layer.source.networkOptions)
          .then(function (blob) {
            ...
            while (stack.length && offset < blob.byteLength) {
              ...
              for (var indexChild = 0; indexChild < 8; indexChild++) {
                ...
                if (
                  snode.childrenBitField & (1 << indexChild) &&
                  offset + 5 <= blob.byteLength
                ) {
                  var childrenBitField = view.getUint8(offset);
                  offset += 1;

                  var numPoints =
                    view.getUint32(offset, true) || _this2.numPoints;

                  offset += 4;
                  var item = new PotreeNode(
                    numPoints,
                    childrenBitField,
                    _this2.layer,
                  );
