  ...
  (0, _createClass2['default'])(C3DTBoundingVolume, [
    {
      ...
      value: function initBoundingRegion(region, inverseTileTransform) {
        ...
        regionBox.matrix.decompose(
          regionBox.position,
          regionBox.quaternion,
          regionBox.scale,
        );
    ...
    },
    {
      ...
      value: function boundingVolumeCulling(camera, tileMatrixWorld) {
        ...
        return (
          this.sphere && !camera.isSphereVisible(this.sphere, tileMatrixWorld)
        );
      },
    },
    {
      ...
      value: function viewerRequestVolumeCulling(camera, tileMatrixWorld) {
        ...
        if (this.sphere) {
          ...
          if (
            !(
              camera.camera3D.position.distanceTo(worldCoordinateCenter) <=
              this.sphere.radius
            )
          ) {
            return true;
