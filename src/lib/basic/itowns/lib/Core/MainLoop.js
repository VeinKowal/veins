...
exports['default'] =
  exports.MAIN_LOOP_EVENTS =
  exports.RENDERING_SCHEDULED =
  exports.RENDERING_PAUSED =
    void 0;
...
function updateElements(context, geometryLayer, elements) {
  ...
  try {
    for (_iterator.s(); !(_step2 = _iterator.n()).done; ) {
      ...
      var newElementsToUpdate = geometryLayer.update(
        context,
        geometryLayer,
        element,
      );
...
MainLoop.prototype._update = function (view, updateSources, dt) {
  ...
  try {
    for (_iterator4.s(); !(_step5 = _iterator4.n()).done; ) {
      ...
      if (
        geometryLayer.ready &&
        geometryLayer.visible &&
        !geometryLayer.frozen
      ) {
        view.execFrameRequesters(
          MAIN_LOOP_EVENTS.BEFORE_LAYER_UPDATE,
          dt,
        ...
        view.execFrameRequesters(
          MAIN_LOOP_EVENTS.AFTER_LAYER_UPDATE,
          dt,
          this._updateLoopRestarted,
          geometryLayer,
        );
...
MainLoop.prototype._step = function (view, timestamp) {
  ...
  view.execFrameRequesters(
    MAIN_LOOP_EVENTS.BEFORE_CAMERA_UPDATE,
    dt,
    this._updateLoopRestarted,
  );
