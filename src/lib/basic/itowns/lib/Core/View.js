...
function _preprocessLayer(view, layer, parentLayer) {
  ...
  if (layer.isGeometryLayer) {
    ...
    if (
      parentLayer &&
      parentLayer.tileMatrixSets &&
      parentLayer.tileMatrixSets.includes(
        _Crs['default'].formatToTms(source.crs),
      )
    ) {
...
var View = /*#__PURE__*/ (function (_THREE$EventDispatche) {
  ...
  function View(crs, viewerDiv) {
    ...
    _this.mainLoop =
      options.mainLoop ||
      new _MainLoop['default'](new _Scheduler['default'](), engine);
    ...
    _this.camera = new _Camera['default'](
      _this.referenceCrs,
      _this.mainLoop.gfxEngine.getWindowSize().x,
      _this.mainLoop.gfxEngine.getWindowSize().y,
      options.camera,
    );
    ...
    _this._allLayersAreReadyCallback = function () {
      ...
      if (
        allReady &&
        _this.mainLoop.scheduler.commandsWaitingExecutionCount() == 0 &&
        _this.mainLoop.renderingState == _MainLoop.RENDERING_PAUSED
      ) {
        _this.dispatchEvent({
          type: VIEW_EVENTS.LAYERS_INITIALIZED,
        });

        _this.removeFrameRequester(
          _MainLoop.MAIN_LOOP_EVENTS.UPDATE_END,
          _this._allLayersAreReadyCallback,
        );
      }
    };
    ...
    _this._fullSizeDepthBuffer = new Uint8Array(
      4 * _this.camera.width * _this.camera.height,
    );
  ...
  (0, _createClass2['default'])(View, [
    ...
    },
    {
      ...
      value: function addLayer(layer, parentLayer) {
        ...
        if (duplicate) {
          return layer._reject(
            new Error("Invalid id '".concat(layer.id, "': id already used")),
          );
        }
        ...
        if (parentLayer) {
          if (layer.isColorLayer) {
            ...
            var sumColorLayers = parentLayer.countColorLayersTextures.apply(
              parentLayer,
              (0, _toConsumableArray2['default'])(layerColors).concat([layer]),
            );

            if (
              sumColorLayers <=
              (0, _LayeredMaterial.getMaxColorSamplerUnitsCount)()
            ) {
            ...
            layer.source.format == 'image/x-bil;bits=32'
          ) {
            layer.source.networkOptions.isWebGL2 =
          ...
          if (typeof layer.update !== 'function') {
            return layer._reject(
              new Error('Cant add GeometryLayer: missing a update function'),
            );
          }

          if (typeof layer.preUpdate !== 'function') {
            return layer._reject(
              new Error('Cant add GeometryLayer: missing a preUpdate function'),
            );
          }
        ...
        if (
          layer.object3d &&
          !layer.object3d.parent &&
          layer.object3d !== this.scene
        ) {
        ...
        Promise.all(layer._promises).then(function () {
          ...
          if (
            ...
            !_this2._frameRequesters[
              _MainLoop.MAIN_LOOP_EVENTS.UPDATE_END
            ].includes(_this2._allLayersAreReadyCallback)
          ) {
            _this2.addFrameRequester(
              _MainLoop.MAIN_LOOP_EVENTS.UPDATE_END,
        ...
        if (layer) {
          ...
          if (parentLayer && !parentLayer.detach(layer)) {
            throw new Error(
              'Error to detach '
                .concat(layerId, ' from ')
                .concat(parentLayer.id),
            );
          } else if (parentLayer == undefined) {
            // Remove layer from viewer
            this._layers.splice(
              this._layers.findIndex(function (l) {
                return l.id == layerId;
              }),
              1,
            );
    ...
    },
    {
      ...
      value: function removeAllFrameRequesters() {
        for (var when in this._frameRequesters) {
          if (
            Object.prototype.hasOwnProperty.call(this._frameRequesters, when)
          ) {
      ...
      value: function _executeFrameRequestersRemovals() {
        ...
        try {
          for (_iterator9.s(); !(_step9 = _iterator9.n()).done; ) {
            ...
            var index = this._frameRequesters[toDelete.when].indexOf(
              toDelete.frameRequester,
            );
    ...
    },
    {
      ...
      value: function eventToViewCoords(event) {
        ...
        if (event.touches === undefined || !event.touches.length) {
          ...
          return target.set(
            targetBoundingRect.x + event.offsetX - br.x,
    ...
    },
    {
      ...
      value: function eventToNormalizedCoords(event) {
        ...
        return this.viewToNormalizedCoords(
          this.eventToViewCoords(event, _eventCoords, touchIdx),
        );
      },
      /**
      ...
      value: function pickObjectsAt(mouseOrEvt) {
        ...
        for (
          var _len2 = arguments.length,
            where = new Array(_len2 > 2 ? _len2 - 2 : 0),
            _key2 = 2;
          _key2 < _len2;
          _key2++
        ) {
        ...
        var mouse =
          mouseOrEvt instanceof Event
            ? this.eventToViewCoords(mouseOrEvt)
            : mouseOrEvt;
        ...
        for (var _i = 0, _sources = sources; _i < _sources.length; _i++) {
          ...
          if (source.isGeometryLayer) {
            ...
            _Picking['default'].pickObjectsAt(
              this,
              mouse,
              radius,
              source,
    ...
    },
    {
      ...
      value: function getPixelsToMeters() {
        ...
        if (this.camera.camera3D.isOrthographicCamera) {
          screenMeters =
            (this.camera.camera3D.right - this.camera.camera3D.left) /
        ...
        return this.getPixelsToMetersFromDistance(
          pixels,
          this.getDistanceFromCamera(screenCoord),
        );
      },
    },
    {
    ...
    },
    {
      ...
      value: function getMetersToPixels() {
        ...
        return this.getMetersToPixelsFromDistance(
          meters,
          this.getDistanceFromCamera(screenCoord),
        );
      },
      ...
      value: function pickFeaturesAt(mouseOrEvt) {
        ...
        for (
          var _len3 = arguments.length,
            where = new Array(_len3 > 2 ? _len3 - 2 : 0),
            _key3 = 2;
          _key3 < _len3;
          _key3++
        ) {
        ...
        var mouse =
          mouseOrEvt instanceof Event
            ? this.eventToViewCoords(mouseOrEvt, _eventCoords)
            : mouseOrEvt;
        var objects = this.pickObjectsAt.apply(
          this,
          [mouse, radius].concat((0, _toConsumableArray2['default'])(where)),
        );
        ...
        var tiles = _Picking['default'].pickTilesAt(
          this,
          mouse,
          radius,
          this.tileLayer,
        ...
        try {
          for (_iterator11.s(); !(_step11 = _iterator11.n()).done; ) {
            ...
            var _iterator12 = _createForOfIteratorHelper(
                tile.object.material.getLayers(layers),
              ),
              _step12;
              ...
              for (_iterator12.s(); !(_step12 = _iterator12.n()).done; ) {
                ...
                try {
                  for (_iterator13.s(); !(_step13 = _iterator13.n()).done; ) {
                    ...
                    precision = _Crs['default'].isMetricUnit(
                      texture.features.crs,
                    )
                      ? precisions.M
                      : precisions.D;
                    result[materialLayer.id] = result[materialLayer.id].concat(
                      _FeaturesUtils['default'].filterFeaturesUnderCoordinate(
                        coordinates,
                        texture.features,
                        precision,
                      ),
    ...
    },
    {
      ...
      value: function readDepthBuffer(x, y, width, height, buffer) {
        ...
        var restore = this.tileLayer.level0Nodes.map(function (n) {
          return _RenderMode['default'].push(
            n,
            _RenderMode['default'].MODES.DEPTH,
          );
        });
    ...
    },
    {
      ...
      value: function getPickingPositionFromDepth(mouse) {
        ...
        if (
          !this.tileLayer ||
          this.tileLayer.level0Nodes.length == 0 ||
        ...
        var viewPaused =
          l.scheduler.commandsWaitingExecutionCount() == 0 &&
          l.renderingState == _MainLoop.RENDERING_PAUSED;
