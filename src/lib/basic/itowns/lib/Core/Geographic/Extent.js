...
function getCountTiles(crs, zoom) {
  var sTs =
    schemeTiles.get(_Crs['default'].formatToTms(crs)) ||
    schemeTiles.get('default');
    ...
    [
      ...
      },
      {
        ...
        value: function tiledCovering(crs) {
          if (this.crs == 'EPSG:4326' && crs == _Crs['default'].tms_3857) {
            ...
            var extent = _extent
              .copy(this)
              .as(_Crs['default'].formatToEPSG(crs), _extent2);
            ...
            var zoom =
              ...
              Math.floor(
                Math.log2(
                  Math.round(globalDimension.x / (dimensionTile.x * sTs.x)),
                ),
              );
            ...
            var maxRow =
              Math.ceil(
                ((globalExtent.north - extent.south) / globalDimension.x) *
                  countTiles.y,
              ) - 1;
            ...
            var _zoom = Math.floor(
              Math.log2(
                Math.round(_globalDimension.x / (dimensionTile.x * _sTs.x)),
              ),
            );
            ...
            tmsCoord.y = isInverted
              ? _globalExtent.north - _center.y
              : _center.y - _globalExtent.south;
        ...
        value: function as(crs, target) {
          ...
          if (_Crs['default'].isTms(this.crs)) {
            ...
            dimensionTile
              .set(1, 1)
              .divide(countTiles)
              .multiply(globalDimension);
            target.west =
              globalExtent.west +
              (globalDimension.x - dimensionTile.x * (countTiles.x - this.col));
            target.east = target.west + dimensionTile.x;
            target.south =
              globalExtent.south +
              dimensionTile.y * (countTiles.y - this.row - 1);
        ...
        value: function center() {
          var target =
            arguments.length > 0 && arguments[0] !== undefined
              ? arguments[0]
              : new _Coordinates['default'](this.crs);
      ...
      },
      {
        ...
        value: function isInside(extent, epsilon) {
          if (_Crs['default'].isTms(this.crs)) {
            ...
            epsilon =
              epsilon == undefined
                ? _Crs['default'].reasonnableEpsilon(this.crs)
                : epsilon;
            return (
              this.east - _extent.east <= epsilon &&
      ...
      },
      {
        ...
        value: function intersectsExtent(extent) {
          ...
          var other =
            extent.crs == this.crs ? extent : extent.as(this.crs, _extent);
          return !(
