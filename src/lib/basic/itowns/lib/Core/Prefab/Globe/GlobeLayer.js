...
var _TiledGeometryLayer2 = _interopRequireDefault(
...
var GlobeLayer = /*#__PURE__*/ (function (_TiledGeometryLayer) {
  ...
  function GlobeLayer(id, object3d) {
    ...
    var schemeTile = _Extent.globalExtentTMS
      .get('EPSG:4326')
      .subdivisionByScheme(scheme); // Supported tile matrix set for color/elevation layer

    config.tileMatrixSets = [
      _Crs['default'].tms_4326,
      _Crs['default'].tms_3857,
    ];
    ...
    _this.minSubdivisionLevel =
      _this.minSubdivisionLevel == undefined ? 2 : _this.minSubdivisionLevel;
    _this.maxSubdivisionLevel =
      _this.maxSubdivisionLevel == undefined ? 19 : _this.maxSubdivisionLevel;
    ...
    worldToScaledEllipsoid.premultiply(
      new THREE.Matrix4().makeScale(
        1 / _Ellipsoid.ellipsoidSizes.x,
        1 / _Ellipsoid.ellipsoidSizes.y,
        1 / _Ellipsoid.ellipsoidSizes.z,
      ),
    );
    return _this;
  }
  ...
  (0, _createClass2['default'])(GlobeLayer, [
    {
      ...
      value: function preUpdate(context, changeSources) {
        ...
        cameraPosition
          ...
          .applyMatrix4(worldToScaledEllipsoid);
        magnitudeSquared = cameraPosition.lengthSq() - 1.0;
        return (0, _get2['default'])(
          (0, _getPrototypeOf2['default'])(GlobeLayer.prototype),
          'preUpdate',
          this,
        ).call(this, context, changeSources);
      },
    },
    {
      ...
      value: function countColorLayersTextures() {
        ...
        for (
          var _len = arguments.length, layers = new Array(_len), _key = 0;
          _key < _len;
          _key++
        ) {
    ...
    },
    {
      ...
      value: function subdivision(context, layer, node) {
        if (node.level == 5) {
          var row = node.getExtentsByProjection(_Crs['default'].tms_4326)[0]
            .row;
    ...
    },
    {
      ...
      value: function horizonCulling(point) {
        ...
        var isOccluded =
          magnitudeSquared < 0
            ? dot > 0
            : magnitudeSquared < dot &&
              magnitudeSquared < (dot * dot) / vtMagnitudeSquared;
        return isOccluded;
      },
    },
    {
      ...
      value: function computeTileZoomFromDistanceCamera(distance, camera) {
        var preSinus =
          (_LayeredMaterialNodeProcessing.SIZE_DIAGONAL_TEXTURE *
            (this.sseSubdivisionThreshold * 0.5)) /
          camera._preSSE /
          _Ellipsoid.ellipsoidSizes.x;
        var sinus = distance * preSinus;
        var zoom = Math.log(Math.PI / (2.0 * Math.asin(sinus))) / Math.log(2);
        var delta = Math.PI / Math.pow(2, zoom);
        var circleChord =
          2.0 * _Ellipsoid.ellipsoidSizes.x * Math.sin(delta * 0.5);
    ...
    },
    {
      ...
      value: function computeDistanceCameraFromTileZoom(zoom, camera) {
        ...
        return (
          (camera._preSSE *
            (radius / _LayeredMaterialNodeProcessing.SIZE_DIAGONAL_TEXTURE)) /
            (this.sseSubdivisionThreshold * 0.5) +
          radius
        );
      },
