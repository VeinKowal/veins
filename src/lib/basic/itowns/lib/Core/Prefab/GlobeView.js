...
var _GlobeControls = _interopRequireDefault(
...
var GlobeView = /*#__PURE__*/ (function (_View) {
  ...
  function GlobeView(viewerDiv) {
    ...
    _this.camera.camera3D.near = Math.max(
      15.0,
      0.000002352 * _Ellipsoid.ellipsoidSizes.x,
    );
    _this.camera.camera3D.far = _Ellipsoid.ellipsoidSizes.x * 10;
    var tileLayer = new _GlobeLayer['default'](
      'globe',
      options.object3d,
      options,
    );
    ...
    if (!placement.isExtent) {
      placement.coord =
        placement.coord || new _Coordinates['default']('EPSG:4326', 0, 0);
    ...
    if (options.noControls) {
      _CameraUtils['default'].transformCameraToLookAtTarget(
        ...
        _this.camera.camera3D,
        placement,
      );
    } else {
      _this.controls = new _GlobeControls['default'](
        (0, _assertThisInitialized2['default'])(_this),
        placement,
      );
      _this.controls.handleCollision =
    ...
    _this.addLayer(
      new _Atmosphere['default']('atmosphere', options.atmosphere),
  ...
  (0, _createClass2['default'])(GlobeView, [
    {
      ...
      value: function addLayer(layer) {
        ...
        if (layer.isColorLayer) {
          if (
            !this.tileLayer.tileMatrixSets.includes(
              _Crs['default'].formatToTms(layer.source.crs),
            )
          ) {
            return layer._reject(
              'Only '.concat(
                this.tileLayer.tileMatrixSets,
          ...
          if (
            _Crs['default'].formatToTms(layer.source.crs) !==
            this.tileLayer.tileMatrixSets[0]
          ) {
            return layer._reject(
              'Only '.concat(
                this.tileLayer.tileMatrixSets[0],
        ...
        return (0, _get2['default'])(
          (0, _getPrototypeOf2['default'])(GlobeView.prototype),
          'addLayer',
          this,
    ...
    },
    {
      ...
      value: function getPixelsToDegrees() {
        ...
        return this.getMetersToDegrees(
          this.getPixelsToMeters(pixels, screenCoord),
        );
      },
    },
    {
      ...
      value: function getPixelsToDegreesFromDistance() {
        ...
        return this.getMetersToDegrees(
          this.getPixelsToMetersFromDistance(pixels, distance),
        );
      },
    },
    {
      ...
      value: function getMetersToDegrees() {
        ...
        return THREE.MathUtils.radToDeg(
          2 * Math.asin(meters / (2 * _Ellipsoid.ellipsoidSizes.x)),
        );
      },
    },
  ]);
