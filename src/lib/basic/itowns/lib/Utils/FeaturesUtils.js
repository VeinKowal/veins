...
function pointIsInsidePolygon(point, polygonPoints, offset, count, size) {
  ...
  for (
    var i = offset, j = offset + count - size;
    i < offset + count;
    j = i, i += size
  ) {
...
function isFeatureSingleGeometryUnderCoordinate(
  coordinate,
  type,
  coordinates,
  epsilon,
  ...
  if (
    type == _Feature.FEATURE_TYPES.LINE &&
    pointIsOverLine(coordinate, coordinates, epsilon, offset, count, size)
  ) {
    return true;
  } else if (
    type == _Feature.FEATURE_TYPES.POLYGON &&
    pointIsInsidePolygon(coordinate, coordinates, offset, count, size)
  ) {
    return true;
...
function isFeatureUnderCoordinate(coordinate, feature, epsilon, result) {
  ...
  try {
    for (_iterator.s(); !(_step = _iterator.n()).done; ) {
      ...
      if (
        geometry.extent == undefined ||
        geometry.extent.isPointInside(featCoord, epsilon)
      ) {
        var offset = geometry.indices[0].offset * feature.size;
        var count = geometry.indices[0].count * feature.size;
        var under = isFeatureSingleGeometryUnderCoordinate(
          featCoord,
          feature.type,
          feature.vertices,
          epsilon,
...
var _default = {
   ...
   */
  filterFeaturesUnderCoordinate: function filterFeaturesUnderCoordinate(
    coordinate,
    collection,
  ) {
    var epsilon =
    ...
    if (collection.extent.isPointInside(coord, epsilon)) {
      if (collection.isFeatureCollection) {
        epsilon *= Math.sqrt(
          Math.pow(collection.scale.x, 2) + Math.pow(collection.scale.y, 2),
        );
        ...
        try {
          for (_iterator2.s(); !(_step2 = _iterator2.n()).done; ) {
            ...
            if (
              feature.extent &&
              !feature.extent.isPointInside(coord, epsilon)
            ) {
              continue;
            }
