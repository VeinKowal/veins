...
var _LayerUpdateState = _interopRequireDefault(
  require('../Layer/LayerUpdateState'),
);
...
var SIZE_DIAGONAL_TEXTURE = Math.pow(
  2 * (SIZE_TEXTURE_TILE * SIZE_TEXTURE_TILE),
  0.5,
);
...
function refinementCommandCancellationFn(cmd) {
  ...
  if (
    cmd.layer.isElevationLayer &&
    cmd.requester.material.getElevationLayer() &&
  ...
  if (
    !cmd.requester.layerUpdateState[cmd.layer.id] ||
...
function updateLayeredMaterialNodeImagery(context, layer, node, parent) {
  ...
  if (node.layerUpdateState[layer.id] === undefined) {
    ...
    if (!layer.source.extentInsideLimit(node.extent, zoom)) {
      ...
      if (
        !(
          !layer.noTextureParentOutsideLimit &&
          parent.material &&
  ...
  var targetLevel = (0, _LayerUpdateStrategy.chooseNextLevelToFetch)(
    layer.updateStrategy.type,
    node,
    destinationLevel,
    nodeLayer.level,
    layer,
  ...
  if (
    (!layer.source.isVectorSource && targetLevel <= nodeLayer.level) ||
    targetLevel > destinationLevel
  ) {
  ...
  return context.scheduler.execute(command).then(
    ...
    function (err) {
      return (0, _handlerNodeError['default'])(
        err,
        node,
        layer,
        targetLevel,
...
function updateLayeredMaterialNodeElevation(context, layer, node, parent) {
  ...
  if (
    targetLevel <= nodeLayer.level ||
    targetLevel > extentsDestination[0].zoom
  ) {
    node.layerUpdateState[layer.id].noMoreUpdatePossible();
    return;
  } else if (!layer.source.extentInsideLimit(node.extent, targetLevel)) {
