...
var EntwinePointTileSource = /*#__PURE__*/ (function (_Source) {
  ...
  function EntwinePointTileSource(config) {
    ...
    _this.whenReady = _Fetcher['default']
      .json(''.concat(_this.url, '/ept.json'), _this.networkOptions)
      .then(function (metadata) {
        // Set parser and its configuration from schema
        _this.parse =
          metadata.dataType === 'laszip'
            ? _LASParser['default'].parse
        ...
        if (metadata.srs && metadata.srs.authority && metadata.srs.horizontal) {
          _this.crs = ''
            .concat(metadata.srs.authority, ':')
            .concat(metadata.srs.horizontal);
          ...
          if (
            metadata.srs.vertical &&
            metadata.srs.vertical !== metadata.srs.horizontal
          ) {
            console.warn(
              'EntwinePointTileSource: Vertical coordinates system code is not yet supported.',
            );
          }
        ...
        _this.spacing =
          (Math.abs(
            metadata.boundsConforming[3] - metadata.boundsConforming[0],
          ) +
            Math.abs(
              metadata.boundsConforming[4] - metadata.boundsConforming[1],
            )) /
          (2 * metadata.span);
