...
var CSVnVRTParser = (function _() {
  ...
  function xml2json(xml, json) {
    ...
    if (attributes.length > 0) {
      ...
      for (var i = 0; i < attributes.length; i++) {
        res['@attributes'][attributes[i]] = xml.getAttributeNode(
          attributes[i],
        ).value;
      }
    }
  ...
  function getGeometryType(type) {
    switch (type) {
      ...
      default:
        throw new Error(
          'This type of GeometryType is not supported yet: ' + type,
        );
    }
  }
  ...
  function OGRVRTLayer2Feature(layer, data, crs, options) {
    ...
    if (layer.Field) {
      ...
      for (var f = 0; f < layer.Field.length; f++) {
        layer.Field[f]['@attributes'].pos = header.indexOf(
          layer.Field[f]['@attributes'].src,
        );
      }
    }
    ...
    if (layer.GeometryField) {
      switch (layer.GeometryField['@attributes'].encoding) {
          ...
          for (var i = 0; i < data.length; i++) {
            ...
            if (layer.Field) {
              for (var p = 0; p < layer.Field.length; p++) {
                geometry.properties[layer.Field[p]['@attributes'].name] =
                  line[layer.Field[p]['@attributes'].pos];
              }
            }

            geometry.startSubGeometry(1, feature);
            coord.crs =
              (layer.GeometryField.SRS && layer.GeometryField.SRS.value) ||
              _crs;
            coord.setFromValues(
              Number(line[x]),
              Number(line[y]),
        ...
        default:
          throw new Error(
            'This type of encoding is not supported yet: ' +
              layer.GeometryField['@attributes'].encoding,
          );
      }
    }
  ...
  function readLayer(layer, data, options, crs) {
    if (layer.OGRVRTLayer) {
      return OGRVRTLayer2Feature(
        layer.OGRVRTLayer,
        data,
        layer.TargetSRS.value,
        options,
      );
