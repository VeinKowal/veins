...
var DragNDrop = (function _() {
  ...
  function addFiles(event, files) {
    ...
    for (var i = 0; i < files.length; i++) {
      ...
      fileReader.onload = function onload(e) {
        ...
        extension
          .parser(data, {
            ...
            out: {
              crs:
                extension.mode == _GEOMETRY
                  ? _view.referenceCrs
                  : _view.tileLayer.extent.crs,
              buildExtent: true,
              mergeFeatures: true,
          ...
          .then(function _(features) {
            ...
            if (extension.mode == _COLOR) {
              ...
              throw new Error(
                'Mode of file not supported, please add it using DragNDrop.register',
              );
            }
      ...
      switch (extension.type) {
        ...
        default:
          throw new Error(
            'Type of file not supported, please add it using DragNDrop.register',
          );
      }
    }
  }
  ...
  document.addEventListener(
    'drop',
    function _(e) {
      addFiles(e, e.dataTransfer.files);
    },
    false,
  );
  document.addEventListener(
    'paste',
    function _(e) {
      addFiles(e, e.clipboardData.files);
    },
    false,
  );
