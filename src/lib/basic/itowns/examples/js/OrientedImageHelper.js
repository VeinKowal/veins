...
function transformTexturedPlane(camera, distance, plane) {
  var Yreel =
    2 * Math.tan(itowns.THREE.MathUtils.degToRad(camera.fov / 2)) * distance;
...
function initCamera(
  ...
  var fov = itowns.THREE.MathUtils.radToDeg(
    2 * Math.atan(size[1] / 2 / focale),
  );
  var coordView;
  var localSpace;
  var orientedImage;
  ...
  camera = new itowns.THREE.PerspectiveCamera(
    fov,
    size[0] / size[1],
    distance / 2,
    distance * 2,
...
function setupPictureUI(
  ...
  orientedImageGUI
    ...
    .name('Distance')
    .onChange(function distanceChanged(value) {
  ...
  orientedImageGUI
    ...
    .name('Opacity')
    .onChange(function opacityChanged(value) {
